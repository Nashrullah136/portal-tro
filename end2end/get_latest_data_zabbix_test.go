...
func Test_get_latest_data_zabbix(t *testing.T) {
	testData, err := testutil.ReadYamlFile("get_latest_data_zabbix.yaml")
	if err != nil {
		t.Fatal(err)
	}
	for _, data := range testData {
		t.Run(data.Name, func(t *testing.T) {
			...
			responseBody := e.GET("/server-utilization/latest-data").WithHeaders(auth).
